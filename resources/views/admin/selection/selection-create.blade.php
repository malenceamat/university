...
@section('selection')
    ...
    <div class="profile-image">
        <form @if (isset($selection->id)) action="{{url('selection/edit')}}"
              @else action="{{url('selection/save')}}" @endif method="post"
                ...
                <div class="mb-3">
                    <div class="col-lg-5 col-sm-12 mt-3 pl-0">
                        <input id="itn-croppie-result-i" type="text" hidden value=""
                               name="image">
                        <img id="itn-croppie-result" src="{{asset('/storage/'.$selection['image'])}}"
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            ...
                                            <input type="text" class="form-control mb-3"
                                                   placeholder="Текст кнопки" id="button"
                                                   name="button" value="{{$selection['button']}}">
                                        </div>
                                    </div>
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            <label for="url">Ссылка кнопки</label>
                                            <input id="url" type="url" name="url" placeholder="https://dummyurl.com"
                                                   class="form-control" value="{{$selection['url']}}" required>
                                        </div>
                                    </div>
        ...
        @if(isset($selection->id))
            <div class="" style="float: right;height: auto">
                <form method="POST" action="/selection/{{$selection->id}}">
                    @csrf
                    {{method_field('DELETE')}}
